...
class MediaPlayerImp : IMediaPlayer
{
    ...
    private MediaPlayerImp(string appId, MediaPlayerSourceEvent sourceEvent)
    {
        ...
        IMediaPlayerNative.media_player_registerPlayerSourceObserver(_media_player, OnPlayerSourceStateChanged);
    }

    private EventListener InitHandler()
    {
        _source_event_native = new EventListener { _OnPlayerSourceStateChanged = OnPlayerSourceStateChanged,
                                                   ...
                                                   _OnPlayBufferUpdated = OnPlayerBufferUpdated,

                                                   _OnPreloadEvent = OnPreloadEvent,

                                                   _OnCompleted = OnCompleted,

                                                   _OnAgoraCDNTokenNeedRenew = OnAgoraCDNTokenNeedRenew };

        return _source_event_native;
    }

    [AOT.MonoPInvokeCallback(typeof(FUNC_onPlayerSourceStateChanged))]
    private static void OnPlayerSourceStateChanged(MEDIA_PLAYER_STATE state, MEDIA_PLAYER_ERROR ec)
    {
