...
class MediaPlayerImp : IMediaPlayer
{
    ...
    private EventListener InitHandler()
    {
        _source_event_native = new EventListener { 
            test = 10,
            _OnPlayerSourceStateChanged = Marshal.GetFunctionPointerForDelegate(OnPlayerSourceStateChanged)
    ...
    public delegate static void OnPlayerSourceStateChanged(int state, int ec)
    {
