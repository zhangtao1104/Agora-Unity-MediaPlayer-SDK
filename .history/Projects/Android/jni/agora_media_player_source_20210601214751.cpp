...
AgoraMediaPlayerSourceObserver::AgoraMediaPlayerSourceObserver(
    EventListener *event) {
  _event = event;
}
...
void AgoraMediaPlayerSourceObserver::onPlayBufferUpdated(
...
void AgoraMediaPlayerSourceObserver::onFrame(
    const media::base::VideoFrame *frame) {
  PRINTF("AgoraMediaPlayerSourceObserver::onFrame");
  if (_event && _event->_onFrame) {
    PRINTF("AgoraMediaPlayerSourceObserver::onFrame width: %d, height: %d, "
           "yStride: %d, rotation: %d",
           frame->width, frame->height, frame->yStride, frame->rotation);
